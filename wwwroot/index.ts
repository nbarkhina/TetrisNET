declare var window, rivets, navigator, $, signalR, toastr;
...
export class MyApp {
    ...
    //network variables
    signalrConnection:any; //the connection object
    player1Status:string = ''; //to see if player is connected
    player2Status:string = ''; //to see if player is connected
    player1:Player = new Player(); //player object
    player2:Player = new Player(); //player object
    networkStatus:string = ''; //used for various status messages
    playerNum:number = 0; //which player is this 1 or 2
    networkConsole:boolean = false; //print network stats to console
    competitorBoard:string = '';
    garbageQueue:number[] = [];
    gameScore:number = 0;
    ...
    async setupSignalRConnection(){
        ...
        this.signalrConnection.on("ReceiveMessage", function (message, details) {
            ...
            if (message == "PlayerDisconnected") {
                ...
                if (disconnectedPlayer == "1") app.player1Status = '';
                if (disconnectedPlayer == "2") app.player2Status = '';
        ...
        //used to start the game
        this.signalrConnection.on("GameOver", function (playerWhoLost:number) {
        ...
        this.signalrConnection.on("ReceiveBoard", function (board:string) {
            // console.log('board received', board);
            app.competitorBoard = board;
        });

        //when receiving competitor's garbage
        this.signalrConnection.on("ReceiveGarbage", function (numLines:number) {
        ...
        this.signalrConnection.on("Connected", function (status,player:Player) {
            if (status=="Success"){
        ...
        this.signalrConnection.on("AllClientsConnected", function (player1:Player,player2:Player) {
            app.player1 = player1;
            app.player2 = player2;
    ...
    sendGarbage(linesCleared:number){

        let garbageToSend = 0;
        if (linesCleared==2) garbageToSend = 1;
        if (linesCleared==3) garbageToSend = 2;
        if (linesCleared==4) garbageToSend = 4;
    ...
    onWindowResize()
    {
        ...
        let myElements:HTMLElement[] = [];
        let myElementsCompetitor:HTMLElement[] = [];
